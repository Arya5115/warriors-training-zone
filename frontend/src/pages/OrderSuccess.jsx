    ...
    <div className="max-w-2xl mx-auto px-6 py-16 text-center">
      ...
      {order && (
        <div className="mt-8 p-6 rounded-2xl border border-border text-left inline-block min-w-[300px]">
          ...
          <div className="font-bold text-red-500 text-2xl">{formatCurrency(order.total)}</div>
        </div>
      )}
      <div className="mt-8 flex gap-3 justify-center">
        <Link to="/dashboard/orders">
          <Button className="btn-primary rounded-full" data-testid="view-orders-btn">
            <Package className="w-4 h-4 mr-2" /> Track Order
          </Button>
        </Link>
        <Link to="/marketplace">
          <Button variant="outline" className="rounded-full" data-testid="continue-shopping">
