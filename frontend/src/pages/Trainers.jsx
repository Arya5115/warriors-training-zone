    ...
    <div className="max-w-7xl mx-auto px-6 md:px-8 py-16">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
        {trainers.map((t) => (
          <div
            key={t.id}
            className="rounded-3xl overflow-hidden border border-border bg-card group hover:border-yellow-500/40 transition-colors"
            ...
            <div className="p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {t.certifications.map((c) => (
                  <span key={c} className="text-[10px] px-2 py-1 rounded-full bg-red-500/10 text-red-500 uppercase tracking-widest">
                    {c}
                  </span>
                ))}
              </div>
              <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
                <LangIcon className="w-3 h-3" /> {t.languages.join(" · ")}
              </div>
              <div className="mt-4 flex items-center justify-between pt-4 border-t border-border">
                <div className="text-sm">
                  <span className="font-bold text-lg">{formatCurrency(t.hourly_rate)}</span>{" "}
                  <span className="text-muted-foreground">/hr</span>
                </div>
                <button className="btn-primary rounded-full px-4 py-2 text-sm font-semibold" data-testid={`book-trainer-${t.id}`}>
                  Book Session
                </button>
              </div>
            </div>
