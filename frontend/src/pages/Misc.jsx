  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-8 py-16">
      <div className="text-center max-w-2xl mx-auto">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">Services</div>
        <h1 className="font-display text-5xl md:text-6xl">
          Programs that <span className="gold-text">deliver.</span>
        </h1>
      </div>
      <div className="grid md:grid-cols-3 gap-6 mt-12">
        {services.map((s) => (
          <div
            key={s.id}
            className="rounded-3xl overflow-hidden border border-border bg-card hover:border-red-500/40 transition-colors"
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="mt-4 flex items-center justify-between">
                <span className="font-bold text-red-500 text-xl">{formatCurrency(s.price)}</span>
                <span className="text-xs text-muted-foreground">
                  {s.duration_min ? `${s.duration_min} min` : "Program"}
                </span>
              </div>
            </div>
          </div>
...
export function BMI() {
  ...
  const calc = async () => {
    try {
      const { data } = await api.post("/bmi", { height_cm: parseFloat(h), weight_kg: parseFloat(w) });
      setRes(data);
    } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      <div className="text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">Tool</div>
        <h1 className="font-display text-5xl">BMI Calculator</h1>
      </div>
      <div className="mt-10 rounded-3xl border border-border p-8 bg-card">
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs uppercase tracking-widest">Height (cm)</label>
            <Input type="number" value={h} onChange={(e) => setH(e.target.value)} data-testid="bmi-height" className="mt-1" />
          </div>
          <div>
            <label className="text-xs uppercase tracking-widest">Weight (kg)</label>
            <Input type="number" value={w} onChange={(e) => setW(e.target.value)} data-testid="bmi-weight" className="mt-1" />
          </div>
        </div>
        <Button onClick={calc} className="btn-primary rounded-full mt-6 w-full h-11" data-testid="bmi-calc">
          Calculate BMI
        </Button>
        {res && (
...
export function FAQs() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      <div className="text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">FAQ</div>
        <h1 className="font-display text-5xl">Questions? We got you.</h1>
      </div>
      <Accordion type="single" collapsible className="mt-10">
        {faqs.map((f) => (
          <AccordionItem key={f.id} value={f.id} className="rounded-2xl border border-border px-5 mb-3">
            <AccordionTrigger className="text-left" data-testid={`faq-${f.id}`}>
              {f.question}
            </AccordionTrigger>
...
export function Blogs() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-8 py-16">
      <div className="text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">Blog</div>
        <h1 className="font-display text-5xl md:text-6xl">
          Knowledge is <span className="gold-text">power.</span>
        </h1>
      </div>
      <div className="grid md:grid-cols-3 gap-6 mt-12">
        {blogs.map((b) => (
...
export function Events() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-8 py-16">
      <div className="text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">Events</div>
        <h1 className="font-display text-5xl md:text-6xl">
          Compete. Learn. <span className="crimson-text">Grow.</span>
        </h1>
      </div>
      <div className="grid md:grid-cols-3 gap-6 mt-12">
        {events.map((e) => (
          <div key={e.id} className="rounded-3xl overflow-hidden border border-border bg-card">
            ...
            <div className="p-6">
              ...
              <div className="text-xs text-muted-foreground mt-3">Location: {e.location}</div>
            </div>
          </div>
        ))}
      </div>
...
export function Contact() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      <div className="text-center">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">Contact</div>
      ...
      {sent ? (
        <div className="mt-10 rounded-3xl bg-green-500/10 text-green-700 border border-green-500/30 p-8 text-center">
          ...
          <p className="mt-2">A Warriors coach will reach out within 24 hours.</p>
        </div>
      ) : (
        <form onSubmit={submit} className="mt-10 rounded-3xl border border-border p-8 bg-card space-y-4">
          ...
          <Button type="submit" className="btn-primary rounded-full w-full h-11" data-testid="contact-submit">
            Send Message
          </Button>
        </form>
      )}
...
export function About() {
  return (
    <div className="max-w-5xl mx-auto px-6 py-16">
      <div className="text-center max-w-2xl mx-auto">
        <div className="text-xs uppercase tracking-[0.3em] text-red-500 mb-2">About</div>
        <h1 className="font-display text-5xl md:text-6xl">
          Built by <span className="crimson-text">athletes.</span> For <span className="gold-text">warriors.</span>
        </h1>
      </div>
