...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      i += 1;
      setCounters({ members: Math.min(1250, i * 25), trainers: Math.min(35, Math.floor(i / 2)), hours: Math.min(85000, i * 1700) });
  ...
  return (
    <div>
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-8 py-24 md:py-36">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="font-display text-5xl sm:text-7xl md:text-8xl leading-[0.9] text-white max-w-4xl">
              Forge the <span className="crimson-text">warrior</span><br />
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4">
              <Link to="/membership"><Button size="lg" className="btn-primary rounded-full text-base px-8" data-testid="hero-join-btn">Join Now <ArrowRight className="ml-2 w-4 h-4" /></Button></Link>
              <Link to="/services"><Button size="lg" variant="outline" className="rounded-full text-white border-white/30 hover:bg-white/10 hover:text-white bg-transparent" data-testid="hero-trial-btn"><Play className="mr-2 w-4 h-4" /> Book Free Trial</Button></Link>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 py-20 grid md:grid-cols-12 gap-10 items-center">
        <div className="md:col-span-5 relative">
          ...
          <div className="absolute -bottom-6 -right-6 glass rounded-2xl p-5 max-w-[220px]">
            <div className="flex items-center gap-2 mb-1"><Trophy className="w-4 h-4 text-yellow-500" /><span className="text-xs uppercase tracking-widest">Since 2016</span></div>
      ...
      <section className="bg-muted/40 py-20">
        <div className="max-w-7xl mx-auto px-6 md:px-8">
          <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
            ...
            <Link to="/trainers" className="text-sm font-semibold hover:text-red-500 transition-colors flex items-center gap-1">See all trainers <ArrowRight className="w-4 h-4" /></Link>
          </div>
              ...
              <div key={t.id} className="group rounded-3xl overflow-hidden border border-border bg-card hover:border-yellow-500/40 transition-all">
                <div className="relative aspect-[4/5] overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 text-white">
                    ...
                    <div className="flex items-center gap-1 mt-1"><Star className="w-4 h-4 fill-yellow-400 text-yellow-400" /><span className="text-sm">{t.rating}</span><span className="text-xs text-neutral-400 ml-2">{t.experience_years} yrs exp</span></div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 py-20 text-center">
        ...
        <p className="text-muted-foreground mt-3 max-w-xl mx-auto">From weekend warriors to competitive athletes â a tier for every commitment level.</p>
        <div className="mt-10">
          <Link to="/membership"><Button size="lg" className="btn-gold rounded-full px-8" data-testid="home-view-plans">View Plans</Button></Link>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-8 py-20">
        <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
          ...
          <Link to="/marketplace" className="text-sm font-semibold hover:text-red-500 transition-colors flex items-center gap-1">Shop all <ArrowRight className="w-4 h-4" /></Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {products.map((p) => (
            <Link key={p.id} to={`/product/${p.id}`} className="group rounded-2xl border border-border overflow-hidden bg-card hover:border-red-500/40 transition-colors" data-testid={`home-product-${p.id}`}>
              ...
              <div className="p-4">
                ...
                <div className="mt-2 flex items-center gap-2">
                  <span className="font-bold text-red-500">â¹{Math.round(p.price * (1 - (p.discount || 0) / 100))}</span>
                  {p.discount > 0 && <span className="text-xs text-muted-foreground line-through">â¹{p.price}</span>}
                </div>
              </div>
            </Link>
          ))}
            ...
            {testimonials.map((t) => (
              <div key={t._id || t.name} className="p-6 rounded-2xl bg-card border border-border">
                <div className="flex text-yellow-500 mb-3">
                  {[...Array(t.rating || 5)].map((_, i) => <Star key={i} className="w-4 h-4 fill-yellow-500" />)}
                </div>
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-red-900 via-black to-black p-10 md:p-16 text-white">
          <div className="max-w-2xl relative z-10">
            ...
            <div className="mt-6 flex gap-3 flex-wrap">
              <Link to="/signup"><Button size="lg" className="btn-gold rounded-full px-8" data-testid="cta-signup">Start Free Trial</Button></Link>
              <Link to="/contact"><Button size="lg" variant="outline" className="rounded-full border-white/30 text-white hover:bg-white/10 hover:text-white bg-transparent">Talk to a Coach</Button></Link>
            </div>
          </div>
