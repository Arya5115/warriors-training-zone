    ...
    <div className="max-w-7xl mx-auto px-6 md:px-8 py-16">
      <div className="text-center max-w-2xl mx-auto">
        ...
        <h1 className="font-display text-5xl md:text-6xl">
          Choose your <span className="crimson-text">warrior path</span>
        </h1>
        <p className="text-muted-foreground mt-4">
          All plans include gym access, locker, showers, and community events.
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
        {plans.map((p) => (
          <div
            key={p.id}
            className={`rounded-3xl p-6 flex flex-col shadow-xl ${cardClass(p.tier)}`}
            data-testid={`plan-card-${p.id}`}
          >
            <div className="flex items-center justify-between">
              <div className="text-xs uppercase tracking-[0.25em] opacity-80">{p.tier}</div>
              {p.tier === "gold" && (
                <div className="text-xs px-2 py-1 rounded-full bg-black/20 backdrop-blur flex items-center gap-1">
