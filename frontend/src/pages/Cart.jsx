...
export default function Cart() {
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (cart.subtotal < data.min_amount) {
        return toast.error(`Minimum order ${formatCurrency(data.min_amount)} required`);
      }
      setApplied(data);
  ...
  const discount = applied
    ? applied.discount_type === "percentage"
      ? cart.subtotal * applied.value / 100
      : applied.value
    : 0;
  ...
  if (!cart.items.length) {
    return (
      <div className="max-w-3xl mx-auto px-6 py-24 text-center">
        ...
        <Link to="/marketplace">
          <Button className="btn-primary rounded-full mt-6" data-testid="shop-now-btn">
            Shop Now
          </Button>
        </Link>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-red-500 font-bold mt-1">
                    {formatCurrency(price)} x {it.quantity} = {formatCurrency(price * it.quantity)}
                  </div>
                </div>
                <button
                  onClick={async () => {
                    await cart.removeFromCart(it.product.id);
        ...
        <div className="rounded-2xl border border-border p-6 h-fit sticky top-24">
          ...
          <div className="space-y-2 text-sm">
            <Row label="Subtotal" value={formatCurrency(Math.round(cart.subtotal))} />
            {applied && (
              <Row
                label={`Coupon ${applied.code}`}
                value={`- ${formatCurrency(Math.round(discount))}`}
                highlight
              />
            )}
          ...
          <div className="mt-4 flex gap-2">
            ...
            />
            <Button variant="outline" onClick={applyCoupon} data-testid="apply-coupon">
              Apply
            </Button>
          </div>
          <div className="mt-2 text-xs text-muted-foreground">
            Try: <code className="text-yellow-600">WARRIOR10</code>,{" "}
            <code className="text-yellow-600">IRON20</code>
          </div>
          <Button
            onClick={checkout}
            className="btn-primary w-full rounded-full mt-5 h-11"
