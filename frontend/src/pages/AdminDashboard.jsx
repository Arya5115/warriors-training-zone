...
function Stats() {
  ...
  const cards = [
    { label: "Revenue", value: formatCurrency(s.revenue), color: "from-red-500 to-red-900" },
              ...
              <tr key={u.id} className="border-t border-border" data-testid={`admin-user-${u.id}`}>
                ...
                <td className="p-3">
                  <Select value={u.role} onValueChange={(v) => setRole(u.id, v)}>
                    ...
                    <SelectContent>
                      {["admin", "staff", "trainer", "user"].map((r) => (
    ...
    <div>
      ...
      <div className="rounded-2xl border border-border p-5 mb-6">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <Button onClick={create} className="btn-primary rounded-full" data-testid="add-product-btn">
            Add Product
          </Button>
        </div>
      </div>
      <div className="grid md:grid-cols-3 gap-4">
        {products.map((p) => (
          <div key={p.id} className="rounded-2xl border border-border p-4">
            ...
            <div className="text-xs text-muted-foreground">{p.brand} · {formatCurrency(p.price)} · Stock: {p.stock}</div>
            <Button size="sm" variant="destructive" onClick={() => del(p.id)} className="mt-2 w-full" data-testid={`del-prod-${p.id}`}>
              Delete
            </Button>
          </div>
        ))}
...
function PaymentsTab() {
  ...
  useEffect(() => {
    api.get("/admin/payments").then((r) => setPays(r.data));
  }, []);
          ...
          <tbody>
            {pays.map((p) => (
              <tr key={p.id} className="border-t border-border">
                ...
                <td className="p-3">
                  <span className="text-xs px-2 py-1 rounded-full bg-green-500/10 text-green-600">{p.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
