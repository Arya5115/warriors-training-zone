            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-colors ${
                  isActive ? "bg-red-500/10 text-red-500" : "hover:bg-muted"
                }`
              }
            >
...
function MembershipTab() {
  ...
  useEffect(() => {
    api.get("/memberships/my").then((r) => setMems(r.data));
  }, []);

  const cardClass = (t) =>
    t === "silver" ? "card-silver" : t === "gold" ? "card-gold" : t === "platinum" ? "card-platinum" : "bg-muted";

  if (!mems.length) {
    return (
      <div className="text-center py-16">
        <p className="text-muted-foreground">No active membership.</p>
        <Link to="/membership" className="btn-primary rounded-full px-6 py-2 inline-block mt-4">Buy Membership</Link>
      </div>
    );
  }
      ...
      {mems.map((m) => (
        <div key={m.id} className={`rounded-3xl p-8 shadow-2xl ${cardClass(m.tier)}`} data-testid={`membership-card-${m.tier}`}>
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            <div>
              <div className="text-xs uppercase opacity-70">Valid Until</div>
              <div className="font-semibold">{new Date(m.expires_at).toLocaleDateString()}</div>
            </div>
            <div>
              <div className="text-xs uppercase opacity-70">Status</div>
              <div className="font-semibold uppercase">{m.status}</div>
            </div>
          </div>
        </div>
      ))}
        ...
        <div key={o.id} className="rounded-2xl border border-border p-5" data-testid={`order-${o.id}`}>
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-lg">{formatCurrency(o.total)}</span>
            </div>
          </div>
...
function WishlistTab() {
  ...
  useEffect(() => {
    api.get("/wishlist").then((r) => setItems(r.data));
  }, []);
      ...
      {items.map((p) => (
        <Link key={p.id} to={`/product/${p.id}`} className="rounded-2xl border border-border overflow-hidden hover:border-red-500/40 transition-colors">
          ...
          <div className="p-4">
            ...
            <div className="text-red-500 font-bold mt-1">{formatCurrency(Math.round(p.price * (1 - (p.discount || 0) / 100)))}</div>
          </div>
        </Link>
      ))}
    </div>
  ...
  return (
    <div className="rounded-2xl border border-border p-6 max-w-lg">
      ...
      <div className="space-y-3 text-sm">
        ...
        <div><span className="text-muted-foreground">Phone:</span> <span className="font-medium ml-2">{user.phone || "--"}</span></div>
